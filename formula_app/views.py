...
def submit(request):
    try:
        ...
        input_value = Input.objects.create(alpha = alpha, betha = betha, phi = phi, delta_a = delta_a, delta_p = delta_p)
        ...
        result = Output.objects.create(foreign_key = input_value, kagh = kagh, kag = kag, kagv = kagv, kaph = kaph, kach = kach, teta_a = teta_a, k0n = k0n, k0t = k0t, k0h = k0h, k0V = k0V, kpgh = kpgh, kpph = kpph, kpch = kpch, teta_p = teta_p)
        outputs = Output.objects.all().values().last()
        return JsonResponse(outputs)
    except:
        return JsonResponse("division by zero")